    ...
    let html = `
        <div class="sticky-info-header">
            <div style="display: flex; align-items: center; justify-content: space-between; width: 100%;">
                <h3>${name}</h3>
                <button class="route-search-btn" onclick="window.open('${searchUrl}', '_blank')" style="font-weight: 300; font-size: 1rem; cursor: pointer; display: flex; align-items: center; gap: 4px; padding: 2px 10px;">
                    경로 <img src="img/search.svg" alt="검색" style="width: 15px; height: 15px;">
                </button>
            </div>
        </div>
...
function updateViewportList() {
    const box = document.getElementById('viewportListBox'); const listContent = document.getElementById('viewportListContent');
    if (box.classList.contains('hidden') || !listContent) return; 
    if (map.getZoom() < 10) { listContent.innerHTML = '<p style="padding:10px; color:#999;">지도를 더 확대하세요.</p>'; document.getElementById('viewportCount').textContent = '현재 화면의 섬 (-)'; return; }
    const bounds = map.getBounds(); 
    
    let visibleIslands = allIslands.filter(island => {
        if (isUsableActive && !checkIsUsable(island)) return false;
        const lat = dmsToDecimal(island.Column23); const lng = dmsToDecimal(island.Column24); if (lat && lng) return bounds.contains([lat, lng]); return false; 
    });
    
    document.getElementById('viewportCount').textContent = `현재 화면의 섬 (${visibleIslands.length})`;
    let html = '';
    if (visibleIslands.length === 0) { html = '<p style="padding:10px; color:#999;">화면 내 섬이 없습니다.</p>'; } else { visibleIslands.forEach(island => { html += `<div class="t-list-item" data-island-id="${island.Column2}"><div class="t-name">${island['무인도서 정보']}</div><div class="t-addr">${formatAddress(island)}</div></div>`; }); }
    listContent.innerHTML = html;
    listContent.querySelectorAll('.t-list-item').forEach(item => {
        item.addEventListener('click', function() {
            const islandId = this.dataset.islandId; const island = allIslands.find(i => i.Column2 === islandId);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('toggleIslandList').onclick = function() {
        const list = document.getElementById('islandList');
        if (list.style.display === 'none') { list.style.display = 'block'; this.textContent = '접기 ▲'; } else { list.style.display = 'none'; this.textContent = '펼치기 ▼'; }
    };
